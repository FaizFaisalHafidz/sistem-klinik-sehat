...
export default function Show({ pendaftaran, posisiAntrian }: Props) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Detail Pendaftaran ${pendaftaran.nomor_pendaftaran}`} />
            ...
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p className="text-gray-600">{pendaftaran.nomor_pendaftaran}</p>
                        </div>
                    </div>
                ...
                {/* Status Alert */}
                <div className={`rounded-lg border p-4 ${getStatusColor(pendaftaran.status)}`}>
                    <div className="flex items-center space-x-3">
                        {pendaftaran.status === 'terdaftar' && <CheckCircle className="w-5 h-5" />}
                        {pendaftaran.status === 'dibatalkan' && <XCircle className="w-5 h-5" />}
                        <div>
                            <p className="font-medium">
                                {pendaftaran.status === 'terdaftar' ? 'Pendaftaran Berhasil' :
                                 pendaftaran.status === 'selesai' ? 'Pemeriksaan Selesai' :
                                 pendaftaran.status === 'dibatalkan' ? 'Pendaftaran Dibatalkan' : 
                                 'Status Tidak Diketahui'}
                            </p>
                            <p className="text-sm">
                                {pendaftaran.status === 'terdaftar' ? 'Pasien telah terdaftar dan masuk dalam antrian' :
                                 pendaftaran.status === 'selesai' ? 'Pemeriksaan telah selesai dilakukan' :
                                 pendaftaran.status === 'dibatalkan' ? 'Pendaftaran telah dibatalkan' :
                            ...
                            <div className="grid grid-cols-2 gap-4 text-sm">
                                ...
                                <div>
                                    ...
                                    <p className="text-xl font-bold">
                                        {new Date(pendaftaran.antrian.estimasi_waktu).toLocaleTimeString('id-ID', {
                                            hour: '2-digit',
                                            minute: '2-digit'
                                        })}
                                    </p>
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <p className="font-medium text-gray-900">
                                    {pendaftaran.pasien.tempat_lahir}, {new Date(pendaftaran.pasien.tanggal_lahir).toLocaleDateString('id-ID')}
                    ...
                    <div className="bg-white rounded-lg border border-gray-200 p-6">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <p className="font-medium text-gray-900">{pendaftaran.nomor_pendaftaran}</p>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <p className="text-sm text-gray-600">Jenis Kunjungan</p>
                                    <p className="font-medium text-gray-900 capitalize">{pendaftaran.jenis_kunjungan}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-600">Status</p>
                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(pendaftaran.status)}`}>
                                        {pendaftaran.status === 'terdaftar' ? 'Terdaftar' :
                                         pendaftaran.status === 'selesai' ? 'Selesai' :
                                         pendaftaran.status === 'dibatalkan' ? 'Dibatalkan' : pendaftaran.status}
                                    </span>
                                </div>
                            </div>
                            <div className="flex items-start space-x-2">
                                ...
                                <div>
                                    <p className="text-sm text-gray-600">Cara Bayar</p>
                                    <p className="font-medium text-gray-900 capitalize">{pendaftaran.cara_bayar}</p>
                                    {pendaftaran.nomor_asuransi && (
                                        <p className="text-sm text-gray-600">No. {pendaftaran.cara_bayar}: {pendaftaran.nomor_asuransi}</p>
                                    )}
                                </div>
                            </div>
                            <div className="flex items-start space-x-2">
                            ...
                            {pendaftaran.keterangan && (
                                <div>
                                    ...
                                    <p className="font-medium text-gray-900">{pendaftaran.keterangan}</p>
                                </div>
                            )}
                            <div className="flex items-start space-x-2">
                            ...
                            <div>
                                ...
                                <p className="font-medium text-gray-900">{pendaftaran.user.nama_lengkap}</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="bg-white rounded-lg border border-gray-200 p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <div className="text-center p-4 bg-gray-50 rounded-lg">
                                ...
                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getAntrianStatusColor(pendaftaran.antrian.status)}`}>
                                    {pendaftaran.antrian.status === 'menunggu' ? 'Menunggu' :
                                     pendaftaran.antrian.status === 'dipanggil' ? 'Dipanggil' :
                                     pendaftaran.antrian.status === 'selesai' ? 'Selesai' :
                                     pendaftaran.antrian.status === 'dibatalkan' ? 'Dibatalkan' : pendaftaran.antrian.status}
                                </span>
                            </div>
                            <div className="text-center p-4 bg-gray-50 rounded-lg">
